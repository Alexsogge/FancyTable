...
class OutputEmulation(OutputDevice):
    ...
    def __init__(self, width: int, height: int, pixel_size):
        ...
        self.window = sg.Window('Window Title', layout, return_keyboard_events=True, use_default_focus=False)
        ...
        for y, row in enumerate(frame_matrix):
            ...
            for x, color in enumerate(row):
                ...
                if self.matrix[y][x] is not None:
                    self.graph.TKCanvas.delete(self.matrix[y][x])
                j = x * self.pixel_size
                self.matrix[y][x] = self.graph.DrawRectangle((j, i), (j + self.pixel_size, i + self.pixel_size), line_color='black',
                                         fill_color=color.rgb_string)
                #self.graph.TKCanvas.itemconfig(self.matrix[y][x], fill=color.rgb_string)
